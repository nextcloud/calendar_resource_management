...
class AdminControllerTest extends TestCase {
	...
	public function testGetBuildingsMapsEntities(): void {
		...
		self::assertSame(Http::STATUS_OK, $response->getStatus());
		self::assertSame([
			['id' => 3, 'name' => 'Headquarters', 'address' => 'Somewhere 1'],
		], $response->getData());
	}

	public function testGetRoomsMapsEquipmentFlags(): void {
		$room = new RoomModel();
		$room->setId(7);
		...
		self::assertSame(2, $data[0]['storyId']);
		self::assertTrue($data[0]['hasVideoConferencing']);
		self::assertTrue($data[0]['isWheelchairAccessible']);
	}
	...
	public function testCreateStoryReturnsCreatedEntity(): void {
		...
		self::assertSame(Http::STATUS_OK, $response->getStatus());
		self::assertSame(['id' => 11, 'name' => 'First floor'], $response->getData());
	}
	...
	public function testCreateRoomReportsUnknownStory(): void {
		...
		$response = $this->controller->createRoom('Meeting room', 404);
	...
	public function testCreateRoomPassesAllPropertiesAndUpdatesTheBackend(): void {
		...
		self::assertSame(['id' => 9, 'name' => 'Meeting room'], $response->getData());
	}
	...
	public function testCreateResourceReturnsCreatedEntity(): void {
		...
		$this->resourceService->expects(self::once())
			...
			->with('Beamer', 1, '', 'default')
			->willReturn($resource);
		$this->resourceManager->expects(self::once())->method('update');

		$response = $this->controller->createResource('Beamer', 1);

		self::assertSame(['id' => 4, 'name' => 'Beamer'], $response->getData());
	}
